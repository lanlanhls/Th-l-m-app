      ...
      <main className="p-4 sm:p-6 lg:p-8">
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center h-[70vh]">
            <LoadingSpinner />
            <p className="mt-4 text-lg text-gray-400 animate-pulse">Đang phân tích dữ liệu và tạo dự báo...</p>
          </div>
        )}
        ...
        {!loading && (
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8 max-w-7xl mx-auto">
            <div className="lg:col-span-3 flex flex-col gap-6">
              <div className="bg-slate-800/60 rounded-xl shadow-lg p-4 border border-slate-700">
                <label htmlFor="date-picker" className="block text-sm font-medium text-gray-300 mb-2">Chọn ngày dự báo:</label>
                 <input 
                  type="date"
                  id="date-picker"
                  ...
                  max={today}
                  className="w-full p-2 rounded-md bg-slate-700 border-slate-600 text-white focus:ring-emerald-500 focus:border-emerald-500"
                />
              </div>
              {riskData && <MapPanel />}
            </div>
